...
def predictK(nombreCuartel):
    # Cargar datos
    file_path_K = f'../modelos/{nombreCuartel}.csv'
    ...
    data_K = data_K[['fecha hora', 'Potasio']]
    
    data_K.drop('fecha hora', axis=1, inplace=True)
    data_K2 = data_K.rolling(window=window_size).mean()

    data_K = pd.concat([data_K[:21], data_K2[21:]], ignore_index=True)

    potasio = data_K['Potasio'].to_numpy()
    ...
    X_test_K, y_test_K = X_potasio[val_split:], y_potasio[val_split:]

    test_dataset_K = TimeSeriesDataset(X_test_K, y_test_K, device)

    batch_size = 128
    test_dataloader_K = DataLoader(test_dataset_K, batch_size=batch_size, shuffle=False)
    ...
    model_N.load_state_dict(torch.load('../modelos/modelo_K.pth', map_location=torch.device('cpu')))
    model_N.eval()


    # Realizar predicciones en el conjunto de prueba
    predictions = inferencia(model_N, test_dataloader_K, device)
