...
const Enviar_foto = ({ file, uploadDate, onClose, onUploadSuccess, tipo }) => {
				...
				setMensaje(
					<div>
						<p className='font-semibold mb-2'>
							La {tipo} se encuentra en estado:{' '}
							<span
								className={`${colorEstado ? 'text-green-600' : 'text-red-600'} ml-2`}
							>
								{data.estado}
							</span>
						</p>
						...
						<p className='text-sm text-gray-600'>
							Confiabilidad:{' '}
							<span className='font-bold'>{data.confiabilidad}</span>
						</p>
					</div>
				);
			}
	...
	return (
		<div>
			...
			{showImage && (
				<div>{/* Aquí iría el componente ImageUploader o similar */}</div>
			)}
