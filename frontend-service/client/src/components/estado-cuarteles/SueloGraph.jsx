...
export default function SueloGraph({ cuartel }) {
	...
	const { data: predictionData } = useGetPredecirNPKQuery(
		predictDays ? { dia: predictDays, cuartel } : skipToken
	);
			...
			try {
				const combinedData = {
					nitrogeno: {
						...
						MAPE: predictionData?.nitrogeno?.MAPE || '',
						explicacion: predictionData?.nitrogeno?.explicación || '',
					},
					fosforo: {
						original: data?.fosforo || [],
						predicciones: predictionData?.fosforo?.predicciones || [],
						MAPE: predictionData?.fosforo?.MAPE || '',
						explicacion: predictionData?.fosforo?.explicación || '',
					},
					potasio: {
						original: data?.potasio || [],
						predicciones: predictionData?.potasio?.predicciones || [],
						MAPE: predictionData?.potasio?.MAPE || '',
						explicacion: predictionData?.potasio?.explicación || '',
					},
				};
				...
				<div className='flex items-center space-x-2'>
					...
					<input
						...
						onChange={e =>
							setDateRange(prev => ({
								...prev,
								start: formatDateForDisplay(e.target.value),
							}))
						}
				...
				<div className='flex items-center space-x-2'>
					...
					<input
						...
						onChange={e =>
							setDateRange(prev => ({
								...prev,
								end: formatDateForDisplay(e.target.value),
							}))
						}
						className='w-40 px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm'
					/>
